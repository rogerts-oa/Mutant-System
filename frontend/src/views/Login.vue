    ...
    <div class="bg-mutant-dark/50 p-12 rounded-[48px] border border-white/5 shadow-2xl w-full max-w-md backdrop-blur-xl">
      <div class="text-center mb-12">
        <div class="w-16 h-16 bg-mutant-neon rounded-2xl mx-auto flex items-center justify-center mb-6 rotate-12 shadow-[0_0_40px_rgba(0,255,0,0.3)]">
          <Dumbbell class="text-black w-8 h-8" />
        </div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-8">
        <div class="relative">
          ...
          <div class="relative">
            <User class="absolute left-5 top-5 w-5 h-5 text-gray-500" />
            <input v-model="user" type="text" class="w-full bg-mutant-black border border-white/5 rounded-2xl p-5 pl-14 focus:border-mutant-neon outline-none transition-all font-bold text-sm" placeholder="ADMIN / STAFF">
          </div>
        </div>
        <div class="relative">
          <label class="block text-[10px] font-black mb-3 text-gray-500 uppercase tracking-[0.3em] ml-2">Contraseña</label>
          <div class="relative">
            <Lock class="absolute left-5 top-5 w-5 h-5 text-gray-500" />
            <input v-model="password" type="password" class="w-full bg-mutant-black border border-white/5 rounded-2xl p-5 pl-14 focus:border-mutant-neon outline-none transition-all font-bold text-sm" placeholder="••••••••">
          </div>
        </div>
        <button type="submit" class="w-full bg-mutant-neon text-black font-black py-5 rounded-2xl hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-mutant-neon/20 uppercase tracking-widest text-xs flex items-center justify-center gap-2">
          <ShieldCheck class="w-4 h-4" /> ENTRAR AL SISTEMA
        </button>
      </form>
...
import { Dumbbell, User, Lock, ShieldCheck } from 'lucide-vue-next';
